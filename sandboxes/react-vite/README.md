# React — Vite + react-router

The ten-line router adapter, a multi-route tour, and the launcher.

> **Generated — do not edit.** This is a standalone copy of
> [`examples/react-vite`](../../examples/react-vite), rewritten so it installs from npm
> instead of the workspace. Change the example, then run `pnpm sandboxes`.

```bash
npm install
npm run dev
```

Open [`src/walkthrough/flows.ts`](src/walkthrough/flows.ts) to see the tour itself — it is
plain data, and every anchor it points at is a typed identifier rather than a
CSS selector.

Then try breaking it: rename an anchor in the registry and run `npx cairnkit check`.
