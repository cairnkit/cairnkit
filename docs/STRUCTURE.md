...
```css
:root {
  --cairn-accent: var(--my-brand-color);
}
```
...
Tailwind, shadcn and Radix are all absent from the _published packages_, each
for its own reason.
...
**Radix** — its Popover and Dialog trap focus. Our whole design depends on the user reaching _past_ the popover to click the real element underneath. We would spend more effort defeating Radix than writing the behaviour.

All three are right for **`examples/next-app`**, and for a future `npx cairnkit add` registry that copies component source into the consumer's repo — where those dependencies already exist and are theirs to own. That gives the shadcn ergonomics without taxing every install.
