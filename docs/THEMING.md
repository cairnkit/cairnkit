...
Cairn renders inside your product, so it defers to your brand rather than
...
```tsx
<h1>Pipeline <TourLauncher flowId="invite-candidate" position="inline" /></h1>
```

Nudge a floating launcher clear of a chat bubble without moving it:

```css
:root { --cairn-launcher-inset: 96px; }
```
...
Anchors and flow ids default to `string`, so Cairn works without any setup.
